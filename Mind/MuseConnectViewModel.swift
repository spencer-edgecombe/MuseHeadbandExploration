...
class MuseConnectViewModel: IXNLogListener, ObservableObject {
  ...
  @Published var shouldConnect: Bool = false {
    didSet {
      if shouldConnect {
        onScanClick()
      } else {
...
//    self.museManager?.startListening()
  ...
  func museListChanged() {
    ...
    DispatchQueue.main.async { [weak self] in
      ...
      guard let muse = self.museManager?.getMuses().first else {
        return
      }
      self.selectedMuse = muse
      self.onConnectClick()
//      self.museNames.removeAll()
...
//      }
  ...
  func onConnectClick() {
    guard let muse = selectedMuse else {
      return
    }
...
//    }
  ...
  func receive(_ packet: IXNMuseConnectionPacket, muse: IXNMuse?) {
    isConnected = packet.currentConnectionState == .connected
